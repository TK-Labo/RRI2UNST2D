...
subroutine unst_rdat(ny_rain, temprain, nx_rain, tt_max_rain,&
    ...
    write(*,'(a27, a)') ' - uum(xspeed)           :', fuum
    write(*,'(a27, a)') ' - vvm(yspeed)           :', fvvm
...
1235 format(13x, 20f10.5)
    ...
    do tt = 0, tt_max_rain
        do i = 1, ny_rain
            do j = 1, nx_rain
                rain(tt, i, j) = temprain(tt, i, j)  ! copy
            enddo
        enddo
    enddo
        ...
        do i = 1, tmp_iqnum
            ...
            if(except_judge==1) then
                ...
                if(rsetsu_i(tmp_inlme(i))/=ny .and. rsetsu_j(tmp_inlme(i))/=nx) then
                    if(qin_j(i,3)==0 .and. dir(rsetsu_i(tmp_inlme(i))+1, rsetsu_j(tmp_inlme(i))+1)==32) then
                ...
                if(rsetsu_i(tmp_inlme(i))/=1 .and. rsetsu_j(tmp_inlme(i))/=nx) then
                    if(qin_j(i,4)==0 .and. dir(rsetsu_i(tmp_inlme(i))-1, rsetsu_j(tmp_inlme(i))+1)==8) then
...
!----------------------------------------------------------------------------
subroutine dsmeshdat(lasth)
    implicit none
    integer, intent(in) :: lasth
    ...
    do i = 1, dsmenum
        ...
        if(dstype(i)==2) then
            ...
            if (dswl_num(i)<=0) then
                ...
                ds_dt(dsme(i)) = lasth * 3600/dswl_num(i)
            endif
        endif
    enddo
