...
subroutine flux
  ...
  do li = 1, link
    ...
    if((inf(lime2) == 0) .or. (inf(lime1) == 0) .or. &
       (rbeta(li) == 0.0d0) .or. &
       (unsth(lime1) <= th .and. dep2 <= th)) then
        um(li) = 0.0d0
        vn(li) = 0.0d0
        lhan(li) = 1
        cycle
    ...
    if(infl(li) == 1) then
        ...
        if (h1 > th) then
            if((h2/h1) <= 2.0d0/3.0d0) then  ! fixed v.1.0.5.1
                um(li) = -sgn * 0.35d0 * h1 * sqrt(2.0d0*gg*h1)*ux(li)
                vn(li) = -sgn * 0.35d0 * h1 * sqrt(2.0d0*gg*h1)*uy(li)
            elseif((h2/h1) > 2.0d0/3.0d0) then  ! fixed v.1.0.5.1
    ...
    if(hhe < baseo(lime1)) then
        if ((inf(lime1)==71 .and. inf(lime2)/=71) .or. &
            (inf(lime2)==71 .and. inf(lime1)/=71)) then  ! change v.1.0.5.1
    ...
    elseif(hhep*hhwp < 0.0d0) then
        if(dep2 > 0.0d0 .or. dep1 > 0.0d0) then
            hhan = hhep - hhwp
            sgn = sign(1.0d0, hhan)
            hh1 = max(hhe, hhw) - max(baseo(lime2), baseo(lime1))  ! fixed 1.0.5.1
            if ((inf(lime1)==71 .and. inf(lime2)/=71) .or. &
    ...
    ! gravitational term
    if(dl(li) /= 0.0d0) then
        u13 = gg*hl(li)*dt2/dl(li)*ux(li)*(dep2 + baseo(lime2) &
            - dep1 - baseo(lime1))
        v13 = gg*hl(li)*dt2/dl(li)*uy(li)*(dep2 + baseo(lime2) &
            - dep1 - baseo(lime1))
    else
        u13 = 0.0d0
        v13 = 0.0d0
    ...
    ! if(plantFN==1) then
    !     vr_theta = (0.5*pi - asin(vr_maxfh_per))*(sqrt(gg*hl(li)*(dep2 + baseo(lime2) &
    !         - dep1 - baseo(lime1)))-vr_startf)
    !     vr_Fd = 0.5d0 * (stem_cd(li) + leaves_cdl(li)*vr_Al(li)/vr_ld(li))* &
    !         vr_dd(li)*(uu(li)**2 + vv(li)**2)*dcos(vr_theta) + &
    !         0.5d0*leaves_csl(li)*(vr_Al(li)*(uu(li)**2 + vv(li)**2)/(vr_l(li)/dcos(vr_theta)))
    !     vr_k = sqx * gg / (vr_N(li)*vr_Fd)
    !     vr_u2 = gg*uu(li)*sqx/(vr_k**2)
    !     vr_v2 = gg*vv(li)*sqx/(vr_k**2)
    ! else
    !     vr_u2 = 0.0d0
    !     vr_v2 = 0.0d0
    ! endif
...
subroutine velocity
    ...
    if(dsmesh==1) then
        if(ids2mesh>0) then  ! fixed v.1.0.5.1
