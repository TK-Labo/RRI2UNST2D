...
!====================================
! Pre(before calculation) Write File
!====================================
...
!1099 format('   unsttime=', f8.0, '(s)', 10f21.7)
    ...
    subroutine prediskwrite(time)
        ...
        !$omp sections
        !$omp section
        call prewrite_array_data(fh_unit, unsth, time, fmt1)

        !$omp section
        call prewrite_array_data(fuum_unit, uum, time, fmt1)
        ...
        !$omp section
        if(paddydam == 1) then
            write(fdhp_unit, 7010) time - (timmax / 2.0d0)
            write(fdhp_unit, 7010) time
        7010 format(' time=', f8.0, '(s)')

            call prewrite_paddyarray_data(fpaddyh_unit, pqh, time, fmt3)
            call prewrite_paddyarray_data(fpaddyq_unit, totalqp, time, fmt2)
        endif
        !$omp end sections
        !$omp end parallel

    end subroutine prediskwrite

end module unst_prewrfile
